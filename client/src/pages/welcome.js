...
// import { saveGuestData } from "../utils/authStorage";
...
function Welcome() {
  ...
  useEffect(() => {
    ...
    async function checkAuth() {
      ...
      // ✅ UPDATED: Check for guest session FIRST
      if (guestUser && userId) {
        // Guest with valid session - allow them to proceed
        navigate("/lobby");
        return;
      }

      // Not a guest - check for registered user token
      if (userId && !guestUser) {
        const result = await verifyAuth();
        if (result.valid) {
          navigate("/lobby");
  ...
  const handleGuestSubmit = () => {
    ...
    if (result) {
      ...
      // ✅ UPDATED: Trigger displayNameChanged event so navbar updates
