...
- `JWT_SECRET` must match Sanskrit-Pictionary server JWT secret.
- `ADMIN_USER_ID` should be the single authorized admin user id.
...
Configure `VITE_AUTH_API_BASE_URL` to point at Sanskrit-Pictionary auth API.
...
| Variable | Description |
| --- | --- |
| `JWT_SECRET` | Used to verify bearer tokens issued by Sanskrit-Pictionary login |
| `ADMIN_USER_ID` | Only this user id is allowed to mutate/read assets via dashboard API |

Optional knobs include branch, manifest path, upload size, allowed origins, and mutation rate limits.
...
Authentication:
- Every `/api/assets` endpoint now requires `Authorization: Bearer <token>`.
- Tokens are obtained by logging in against Sanskrit-Pictionary `/api/auth/login`.
- Access is denied unless token `userId` matches `ADMIN_USER_ID` (or optional `ADMIN_EMAIL`).
- Mutation endpoints are rate-limited and emit audit logs.
...
With auth token (required when API auth is enabled):

```bash
npm run upload:bulk -- --token <jwt-token>
```

Or set `DASHBOARD_AUTH_TOKEN` in environment.
...
## Notes
- Keep `.env` out of git; PAT never belongs in client code.
- This folder is designed to live as a separate module inside Sanskrit-Pictionary so its logic can be extracted later.
