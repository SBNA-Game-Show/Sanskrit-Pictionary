...
const AUTH_TOKEN = (argv.token || process.env.DASHBOARD_AUTH_TOKEN || "").trim();
...
async function main() {
  ...
  console.log(`[bulk-upload] Auth token: ${AUTH_TOKEN ? "provided" : "not provided"}`);
  ...
  const existingIds = skipExisting ? await fetchExistingIds(API_BASE, AUTH_TOKEN) : new Set();
...
async function uploadAsset({ apiBase, authToken, word, assetId, imagePath, audioPath }) {
...
async function fetchExistingIds(apiBase, authToken) {
  try {
    const response = await fetch(`${apiBase}/api/assets`, {
      headers: authToken ? { Authorization: `Bearer ${authToken}` } : {},
    });
