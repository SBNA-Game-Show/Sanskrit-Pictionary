...
const {
  ...
  ADMIN_USER_ID = "",
  ADMIN_EMAIL = "",
  MUTATION_RATE_LIMIT_PER_MIN = "30",
} = process.env;

const securityMissing = [];
if (!JWT_SECRET) securityMissing.push("JWT_SECRET");
if (!ADMIN_USER_ID && !ADMIN_EMAIL) {
  securityMissing.push("ADMIN_USER_ID or ADMIN_EMAIL");
}
if (securityMissing.length) {
  throw new Error(
    `Missing required security environment variables: ${securityMissing.join(", ")}`,
  );
}
...
const mutationRateLimitPerMin = Math.max(
  1,
  Number.parseInt(MUTATION_RATE_LIMIT_PER_MIN, 10) || 30,
);
const mutationRateWindowMs = 60 * 1000;
const mutationTracker = new Map();
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!isAllowedFile(imageFile, VALID_IMAGE_MIME_TYPES)) {
        return res.status(400).json({
          message: "Invalid image file type. Allowed: png, jpg, jpeg, webp",
        });
      }
      ...
      if (audioFile && !isAllowedFile(audioFile, VALID_AUDIO_MIME_TYPES)) {
        return res.status(400).json({
          message: "Invalid audio file type. Allowed: mp3, wav, webm, ogg",
        });
      }
      ...
      logAuditEvent(req, "asset.create", slug);
...
app.patch(
  ...
  async (req, res) => {
    try {
      ...
      if (audioFile) {
        if (!isAllowedFile(audioFile, VALID_AUDIO_MIME_TYPES)) {
          return res.status(400).json({
            message: "Invalid audio file type. Allowed: mp3, wav, webm, ogg",
          });
        }
      ...
      logAuditEvent(req, "asset.update", assetId);
...
app.delete("/api/assets/:assetId", mutationRateLimiter, async (req, res) => {
  try {
    const assetId = (req.params.assetId || "").toString().trim();
    if (!assetId) {
      return res.status(400).json({ message: "assetId is required" });
    ...
    logAuditEvent(req, "asset.delete", assetId);
...
function requireAdmin(req, res, next) {
  ...
  try {
    ...
    if (!isConfiguredAdmin(userId, email)) {
...
function isConfiguredAdmin(userId, email) {
  const adminUserId = (ADMIN_USER_ID || "").trim();
...
function mutationRateLimiter(req, res, next) {
  const now = Date.now();
  const ip = req.ip || req.headers["x-forwarded-for"] || "unknown";
  const key = `${ip}:${req.method}`;
  const bucket = mutationTracker.get(key);

  if (!bucket || now > bucket.resetAt) {
    mutationTracker.set(key, {
      count: 1,
      resetAt: now + mutationRateWindowMs,
    });
    return next();
  }

  bucket.count += 1;
  if (bucket.count > mutationRateLimitPerMin) {
...
function isAllowedFile(file, allowedMimeTypes) {
  const mimeType = (file?.mimetype || "").toLowerCase();
  return Boolean(mimeType) && allowedMimeTypes.has(mimeType);
}

function logAuditEvent(req, action, assetId) {
