...
const AUTH_API_BASE = import.meta.env.VITE_AUTH_API_BASE_URL ?? "http://localhost:5005";
const DASHBOARD_TOKEN_KEY = "image_dashboard_admin_token";
const DASHBOARD_EMAIL_KEY = "image_dashboard_admin_email";
...
function App() {
  const [adminEmail, setAdminEmail] = useState(localStorage.getItem(DASHBOARD_EMAIL_KEY) || "");
  const [adminPassword, setAdminPassword] = useState("");
  const [authToken, setAuthToken] = useState(localStorage.getItem(DASHBOARD_TOKEN_KEY) || "");
  ...
  const getAuthHeaders = (tokenOverride = ""): Record<string, string> => {
  ...
  const handleAdminLogin = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      localStorage.setItem(DASHBOARD_TOKEN_KEY, body.token);
      localStorage.setItem(DASHBOARD_EMAIL_KEY, adminEmail.trim());
      setAuthToken(body.token);
      setAdminPassword("");
      setAuthError("");
      await fetchAssets(body.token);
    } catch (error) {
  ...
  const fetchAssets = async (tokenOverride = "") => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401 || response.status === 403) {
          setAuthError("Unauthorized. Please sign in with the admin account.");
        }
  ...
  }, [filters.q, filters.difficulty, filters.hasAudio, authToken]);
  ...
  if (!authToken) {
    return (
      <div className="dashboard-root">
        <main className="dashboard-main" style={{ maxWidth: 720, margin: "2rem auto" }}>
          <header className="dashboard-header">
            <h1>Sanskrit Pictionary Asset Dashboard</h1>
            <p>Admin login required to manage assets.</p>
          </header>
          <section className="dashboard-section">
            <h2>Admin Sign In</h2>
            <form className="upload-form" onSubmit={handleAdminLogin}>
              ...
              <button className="upload-btn" type="submit" disabled={isAuthLoading}>
